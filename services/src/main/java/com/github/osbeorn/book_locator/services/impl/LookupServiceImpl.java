...
public class LookupServiceImpl implements LookupService {
    ...
    public <T extends BaseLookupType, T2 extends BaseLookupEntity> QueryResult<T> getLookupList(String lookup, QueryParameters queryParameters) {
        Class<T2> sourceClass = getLookupEntityClass(lookup);
        if (sourceClass == null) {
            throw new NoSuchLookupEntityException(lookup);
        }

        Queried<T2> lookupEntitiesQueried = JPAUtils.getQueried(entityManager, sourceClass, queryParameters);
    ...
    public <T> T getLookupEntity(Class<T> clazz, String code) {
        ...
        String libName = entityName.replace("LookupEntity", "");

        if (code == null) {
            throw new ResourceNotFoundException(libName, "null");
        }

        String query = String.format(GET_ONE_QUERY_PATTERN, entityName);
    ...
    public <T> Optional<T> getDefaultLookupEntity(Class<T> clazz) {
        ...
        String query = String.format(GET_DEFAULT_QUERY_PATTERN, entityName);
    ...
    private <T extends BaseLookupEntity> Class<T> getLookupEntityClass(String lookup) {
        ...
        String lookupEntityName = Character.toUpperCase(lookup.charAt(0)) + lookup.substring(1) + "LookupEntity";
        try {
